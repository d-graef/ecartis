...
/* Utility function to read a header line correctly */
int read_mime_line(char *buf, int size, FILE *stream, int inbody)
{
	if (inbody) {
		char *temp = read_file(buf, size, stream);
		if (temp) return 1;
		else return 0;
	} else {
		int i = 0;
		while(1) {
			int a_char = getc_file(stream);
			if (a_char == EOF) {
				if (i < size) {
					buf[i] = '\0';
				} else {
				...
				/* it was a space or tab, just keep going through the loop */
			} else {
				if (i < size) buf[i++] = a_char;
			}
		}
		if (buf[0] != '\0') return 1;
		return 0;
...
MIME_HANDLER(mimehandle_multipart_default)
{
    ...
    while (loop2 ? !done : read_file(templine, sizeof(templine), infile) && !done) {
        ...
        while(!readlast) {
			read_mime_line(templine, sizeof(templine), infile, loop2);
...
MIME_HANDLER(mimehandle_text)
{
    ...
    // Someone might have done some encoding *grrr*
    temp = mime_fieldval(header, "content-transfer-encoding");
    if(temp) {
        if (strcasecmp(temp, "base64") == 0) coding = 1;
        else if (strcasecmp(temp, "quoted-printable") == 0) coding = 2;
    }
    switch (coding) {
        case 2: {
...
void unmime_file(const char *file1, const char *file2)
{
    ...
    while (read_mime_line(templine, sizeof(templine), infile, inbody) && !done) {
...
void unquote_file(const char *file1, const char *file2)
{
    ...
    while(!done && read_mime_line(tbuf, sizeof(tbuf), infile, doneheader)) {
        log_printf(85, "tbuf: .%s.\n", tbuf);
        if(!doneheader &&
